...
export default class Search extends React.Component<SearchProps, SearchState> {
        ...
        if (index.DocumentMetadataConfigurations) {
          this.setState({
            availableSortingAttributes:
              this.state.availableSortingAttributes.fromIndexMetadata(
  ...
  private getResultsHelper = async (
    ...
    const sortingAttribute =
      this.state.selectedSortingAttribute.getSelectedSortingAttribute();
    const sortingOrder =
      this.state.selectedSortingAttribute.getSelectedSortingOrder();
        ...
        if (!isNullOrUndefined(this.props.s3) && result.DocumentURI) {
          ...
          try {
            ...
            if (bucket && key) {
              ...
              console.log(
                "Attempt to convert S3 url to a signed URL: ",
                result.DocumentURI
              );
              ...
              console.log(
                "URL is not an S3 url - return unchanged: ",
                result.DocumentURI
              );
            }
          } catch {
            ...
            console.log(
              "Error signing S3 URL (returning original URL): ",
              result.DocumentURI
            );
          }
          if (result.DocumentURI && excerpt_page_no) {
            result.DocumentURI =
              result.DocumentURI + "#page=" + excerpt_page_no;
          }
    ...
    this.setState(
      {
        selectedSortingAttribute:
          this.state.selectedSortingAttribute.setSelectedSortingAttribute(
  ...
  onSortingOrderChange = (
    ...
    if (sortingOrder === SortOrderEnum.Desc) {
      this.setState(
        {
          selectedSortingAttribute:
            this.state.selectedSortingAttribute.setSelectedSortingOrder(
              SortOrderEnum.Asc
            ),
        },
        () => {
