...
def get_bucket_region(bucket):
    ...
    try:
        region = S3.get_bucket_location(Bucket=bucket)["LocationConstraint"] or 'us-east-2' 
    except Exception as e:
        logger.info(f"Unable to retrieve bucket region (bucket owned by another account?).. defaulting to us-east-2. Bucket: {bucket} - Message: " + str(e))
        region = 'us-east-2'
    return region
